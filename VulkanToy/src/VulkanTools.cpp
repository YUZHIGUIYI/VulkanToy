...
#undef STR
        ...
        VkShaderModule loadShader(const char *fileName, VkDevice device)
        {
            ...
            if (is.is_open())
            {
                ...
                VT_CORE_ASSERT(size > 0, "Shader file can not be empty");

                VkShaderModule shaderModule;
                VkShaderModuleCreateInfo moduleCreateInfo{};
                moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
                moduleCreateInfo.codeSize = size;
                moduleCreateInfo.pCode = (const uint32_t*)shaderCode.data();

                if (vkCreateShaderModule(device, &moduleCreateInfo, NULL, &shaderModule) != VK_SUCCESS)
                    VT_CORE_ASSERT(false, "Fail to create shader module");
            ...
            {
                VT_CORE_ERROR("Could not open shader file \"{}\"", fileName);

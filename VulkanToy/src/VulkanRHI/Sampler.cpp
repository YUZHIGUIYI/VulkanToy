...
namespace VT
{
    ...
    VkSampler SamplerCache::createSampler(const VkSamplerCreateInfo &info, const std::string &samplerName)
    {
        if (m_samplerMap.contains(samplerName))
        {
        ...
        } else
        {
            ...
            m_samplerMap[samplerName] = sampler;
            return sampler;
        }
    }

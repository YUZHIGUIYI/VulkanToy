...
#include <VulkanToy/Renderer/VulkanTexture.h>
...
namespace VT
{
    ...
    ktxResult Texture::loadKTXFile(const std::string &filename, ktxTexture **target)
    {
        ...
        // TODO: make it a tool function
        std::ifstream f(filename);
        if (f.fail())
        {
            VT_CORE_ASSERT(false, "Could not load texture");
        }
	...
	*
	*/
	void Texture2D::loadFromFile(const std::string &filename, VkFormat format, VulkanDevice *device, VkQueue copyQueue,
                                    VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout, bool forceLinear)
    {
        ktxTexture* ktxTexture;
        ktxResult result = loadKTXFile(filename, &ktxTexture);
        VT_CORE_ASSERT(result == KTX_SUCCESS, "Fail to load texture 2D from file");
        ...
        if (useStaging)
        {
            ...
            if (vkCreateBuffer(device->logicalDevice, &bufferCreateInfo, nullptr, &stagingBuffer) != VK_SUCCESS)
            ...
            if (vkAllocateMemory(device->logicalDevice, &memAllocInfo, nullptr, &stagingMemory) != VK_SUCCESS)
                VT_CORE_ASSERT(false, "Fail to allocate memory");
            if (vkBindBufferMemory(device->logicalDevice, stagingBuffer, stagingMemory, 0) != VK_SUCCESS)
            ...
            for (uint32_t i = 0; i < mipLevels; ++i)
            {
                ...
                VT_CORE_ASSERT(result = KTX_SUCCESS, "Fail to get image offset");
            ...
            if (vkCreateImage(device->logicalDevice, &imageCreateInfo, nullptr, &image) != VK_SUCCESS)
                VT_CORE_ASSERT(false, "Fail to create image");
            ...
            if (vkAllocateMemory(device->logicalDevice, &memAllocInfo, nullptr, &deviceMemory) != VK_SUCCESS)
                VT_CORE_ASSERT(false, "Fail to allocate memory");
            if (vkBindImageMemory(device->logicalDevice, image, deviceMemory, 0) != VK_SUCCESS)
                VT_CORE_ASSERT(false, "Fail to bind image memory");
            ...
            // Create mip levels from staging buffer
            ...
            TOOLS::setImageLayout(copyCmd, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, imageLayout, subresourceRange);
        ...
        } else
        {
            ...
            VT_CORE_ASSERT(formatProperties.linearTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT, "Do not support linear tiling");
            ...
            std::memcpy(data, ktxTextureData, memReqs.size);
	...
	*/
    ...
    {
        VT_CORE_ASSERT(buffer, "Buffer can not be nullptr");
	...
	*
	*/
	void Texture2DArray::loadFromFile(const std::string &filename, VkFormat format, VulkanDevice *device,
	...
	*
	*/
    void TextureCubeMap::loadFromFile(const std::string &filename, VkFormat format, VulkanDevice *device,
                                        VkQueue copyQueue, VkImageUsageFlags imageUsageFlags, VkImageLayout imageLayout)
    {
        ktxTexture* ktxTexture;
        ktxResult result = loadKTXFile(filename, &ktxTexture);

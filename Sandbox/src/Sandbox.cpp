...
#include <VulkanToy.h>
#include <VulkanToy/Core/EntryPoint.h>
...
public:
    ...
    // Scene camera
    VT::SceneCamera m_SceneCamera;

    SandBox(VT::ApplicationCommandLineArgs args)
        : VT::Application("Sandbox", args)
    {
        m_SceneCamera = VT::SceneCamera(30.0f, 1.778f, 0.1f, 1000.0f);
    }
    ...
    ~SandBox()
    {
        ...
        vkDestroyBuffer(m_Device, uniformBufferVS.buffer, nullptr);
        vkFreeMemory(m_Device, uniformBufferVS.memory, nullptr);

        vkDestroySemaphore(m_Device, presentCompleteSemaphore, nullptr);
        vkDestroySemaphore(m_Device, renderCompleteSemaphore, nullptr);
    ...
    uint32_t GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties)
    {
        ...
        VT_CORE_ASSERT(false, "Could not find a suitable memory type!");
    }
    ...
    void PrepareSynchronizationPrimitives()
    {
        ...
        vkCreateSemaphore(m_Device, &semaphoreCreateInfo, nullptr, &presentCompleteSemaphore);

        // Semaphore used to ensure that all commands submitted have been finished before submitting the image to the queue
        vkCreateSemaphore(m_Device, &semaphoreCreateInfo, nullptr, &renderCompleteSemaphore);
        ...
        queueCompleteFences.resize(m_DrawCmdBuffers.size());
        for (auto& fence : queueCompleteFences)
        {
    ...
    VkCommandBuffer GetCommandBuffer(bool begin)
    {
        ...
        if (begin)
        {
            VkCommandBufferBeginInfo cmdBufInfo = VT::INITIALIZERS::commandBufferBeginInfo();
    ...
    void BuildCommandBuffers() override
    {
        ...
        for (int32_t i = 0; i < m_DrawCmdBuffers.size(); ++i)
        {
            ...
            vkBeginCommandBuffer(m_DrawCmdBuffers[i], &cmdBufInfo);
            ...
            vkCmdBindDescriptorSets(m_DrawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
            ...
            vkCmdDrawIndexed(m_DrawCmdBuffers[i], indices.count, 1, 0, 0, 1);
    ...
    void Draw()
    {
        ...
        // TODO: Add
        vkResetCommandBuffer(m_DrawCmdBuffers[m_CurrentBuffer], 0);
        RecordCommandBuffer(m_DrawCmdBuffers[m_CurrentBuffer], m_CurrentBuffer);
        ...
        submitInfo.pCommandBuffers = &m_DrawCmdBuffers[m_CurrentBuffer];    // Command buffers(s) to execute in this batch (submission)
        ...
        vkQueueSubmit(m_Queue, 1, &submitInfo, queueCompleteFences[m_CurrentBuffer]);
    ...
    void PrepareVertices(bool useStagingBuffers)
    {
        ...
        if (useStagingBuffers)
        {
            ...
            // Map and copy
            vkMapMemory(m_Device, stagingBuffers.vertices.memory, 0, memAlloc.allocationSize, 0, &data);
            ...
            vkCreateBuffer(m_Device, &vertexBufferInfo, nullptr, &vertices.buffer);
            vkGetBufferMemoryRequirements(m_Device, vertices.buffer, &memReqs);
            memAlloc.allocationSize = memReqs.size;
            memAlloc.memoryTypeIndex = GetMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            vkAllocateMemory(m_Device, &memAlloc, nullptr, &vertices.memory);
            vkBindBufferMemory(m_Device, vertices.buffer, vertices.memory, 0);
            ...
            vkMapMemory(m_Device, stagingBuffers.indices.memory, 0, indexBufferSize, 0, &data);
            ...
            vkCreateBuffer(m_Device, &indexbufferInfo, nullptr, &indices.buffer);
            vkGetBufferMemoryRequirements(m_Device, indices.buffer, &memReqs);
            memAlloc.allocationSize = memReqs.size;
            memAlloc.memoryTypeIndex = GetMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
            vkAllocateMemory(m_Device, &memAlloc, nullptr, &indices.memory);
            vkBindBufferMemory(m_Device, indices.buffer, indices.memory, 0);
        ...
        else
        {
            ...
            memAlloc.memoryTypeIndex = GetMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
            vkAllocateMemory(m_Device, &memAlloc, nullptr, &vertices.memory);
            vkMapMemory(m_Device, vertices.memory, 0, memAlloc.allocationSize, 0, &data);
            std::memcpy(data, vertexBuffer.data(), vertexBufferSize);
            vkUnmapMemory(m_Device, vertices.memory);
            vkBindBufferMemory(m_Device, vertices.buffer, vertices.memory, 0);
            ...
            memAlloc.memoryTypeIndex = GetMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
            vkAllocateMemory(m_Device, &memAlloc, nullptr, &indices.memory);
            vkMapMemory(m_Device, indices.memory, 0, indexBufferSize, 0, &data);
            memcpy(data, indexBuffer.data(), indexBufferSize);
            vkUnmapMemory(m_Device, indices.memory);
            vkBindBufferMemory(m_Device, indices.buffer, indices.memory, 0);
        }
    }
    ...
    {
        // TODO: Remove, descriptor pool has been set up in InitVulkan
    ...
    void SetupDescriptorSetLayout()
    {
        ...
        vkCreateDescriptorSetLayout(m_Device, &descriptorLayout, nullptr, &descriptorSetLayout);
        ...
        vkCreatePipelineLayout(m_Device, &pPipelineLayoutCreateInfo, nullptr, &pipelineLayout);
    }
    ...
    void SetupDepthStencil() override
    {
        ...
        vkCreateImage(m_Device, &image, nullptr, &m_DepthStencil.image);
        ...
        memAlloc.allocationSize = memReqs.size;
        memAlloc.memoryTypeIndex = GetMemoryTypeIndex(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        vkAllocateMemory(m_Device, &memAlloc, nullptr, &m_DepthStencil.mem);
    ...
    void SetupFrameBuffer() override
    {
        ...
        for (size_t i = 0; i < m_FrameBuffers.size(); i++)
        {
            ...
            attachments[0] = m_SwapChain.buffers[i].view; // Color attachment is the view of the swapchain image
            attachments[1] = m_DepthStencil.view;         // Depth/Stencil attachment is the same for all frame buffers
    ...
    void PreparePipelines()
    {
        ...
        vertexInputBinding.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;     // TODO: Input rate
        ...
        shaderStages[0].module = LoadSPIRVShader("../data/shaders/triangleVert.spv");
        ...
        // Load binary SPIR-V shader
        shaderStages[1].module = LoadSPIRVShader("../data/shaders/triangleFrag.spv");
        // Main entry point for the shader
        ...
        vkCreateGraphicsPipelines(m_Device, m_PipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline);
    ...
    void PrepareUniformBuffers()
    {
        ...
        // Create a new buffer
        vkCreateBuffer(m_Device, &bufferInfo, nullptr, &uniformBufferVS.buffer);
        // Get memory requirements including size, alignment and memory type
        vkGetBufferMemoryRequirements(m_Device, uniformBufferVS.buffer, &memReqs);
        ...
        vkAllocateMemory(m_Device, &allocInfo, nullptr, &(uniformBufferVS.memory));
        // Bind memory to buffer
        vkBindBufferMemory(m_Device, uniformBufferVS.buffer, uniformBufferVS.memory, 0);
    ...
    void UpdateUniformBuffers()
    {
        ...
        uboVS.projectionMatrix = m_SceneCamera.GetProjection();
        uboVS.projectionMatrix[1][1] *= -1.0f;
        uboVS.viewMatrix = m_SceneCamera.GetViewMatrix();
        uboVS.modelMatrix = glm::mat4(1.0f);

        // Map uniform buffer and update it
        uint8_t *pData;
        vkMapMemory(m_Device, uniformBufferVS.memory, 0, sizeof(uboVS), 0, (void **)&pData);
        std::memcpy(pData, &uboVS, sizeof(uboVS));
        // Unmap after data has been copied
        // Note: Since we requested a host coherent memory type for the uniform buffer, the write is instantly visible to the GPU
        vkUnmapMemory(m_Device, uniformBufferVS.memory);
    }
    ...
    void Prepare() override
    {
        ...
        PreparePipelines();
        //SetupDescriptorPool();    // Implementation in Application
        SetupDescriptorSet();
        BuildCommandBuffers();
        m_Prepared = true;
    }
...
VT::Application* VT::CreateApplication(ApplicationCommandLineArgs args)
{
